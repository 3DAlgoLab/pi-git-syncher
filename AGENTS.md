...
- If there is no additional change after the dirty flag is on and pi is idle, do `git add` and commit & push. The commit message is generated by the AI: the syncher asks the idle agent to review the staged diff (`git diff --cached`) and the repo's log style, and the agent commits via the `git_syncher_commit` tool (the syncher performs the actual commit & push). One agent request per 30-minute quiet window.
