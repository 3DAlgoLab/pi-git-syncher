...
interface FakePi {
  ...
  commands: Record<string, { description: string; handler: (args: string, ctx: FakeCtx) => Promise<void> }>;
}
...
function makeFakePi(run: (args: string[], cwd: string) => Promise<{ code: number; stdout: string; stderr: string }>): FakePi {
...
test("extension: polls, pulls, toggles, and reports status", async (t) => {
  ...
  await writeFile(join(f.repo, CONFIG_FILE), JSON.stringify({ pollingIntervalMinutes: 0.05 }), "utf8");
  ...
  await waitFor(() => ctx.notes.some((n) => n.message.includes("pulled 1 new commit")), 30_000);
  assert.equal(await readFile(join(f.repo, "pulled.txt"), "utf8"), "remote content\n");
  ...
  assert.ok(statusNote.message.includes(`repo:   ${f.repo}`), statusNote.message);
  assert.ok(statusNote.message.includes("branch: main -> origin/main"), statusNote.message);
  ...
  assert.ok(ctx.notes.some((n) => n.message.includes("OFF")), "toggle-off notification");
  ...
  assert.equal(await readFile(join(f.repo, "while-off.txt"), "utf8").then((s) => s).catch(() => "absent"), "absent");

  // Toggle back on, then the pending remote commit is pulled on a later tick.
  await commands["git-sync"].handler("", ctx);
  assert.ok(ctx.notes.some((n) => n.message.includes("ON")), "toggle-on notification");
  await waitFor(
    () => ctx.notes.some((n, i) => i > notesBefore && n.message.includes("pulled 1 new commit")),
    30_000,
  );
  ...
  await new Promise((r) => setTimeout(r, 6_500));
  assert.equal(ctx.notes.length, notesAtShutdown, "poll loop stopped after shutdown");
});
...
test("extension: /git-sync outside a git repo reports an error", async (t) => {
  ...
  extension(pi);
  await commands["git-sync"].handler("", ctx);
  assert.ok(ctx.notes.some((n) => n.type === "error" && n.message.includes("not a git repository")));
  await commands["git-sync"].handler("status", ctx);
